...
// ==================== MENÚ LATERAL (DESHABILITADO - Ahora usamos menú radial) ====================
// El código del sidebar fue eliminado porque ahora usamos el menú radial

function updateTitle(section) {
  var titleText = document.querySelector('.titulo-arqueado text textPath');
  var titles = {
    'inicio': 'PGOD PUNTOS',
...
// ==================== YOUTUBE VIDEO ====================
const MANUAL_VIDEO_ID = 'x-hvh_59RLo';
...
async function loadLatestVideo() {
  ...
  if (MANUAL_VIDEO_ID && MANUAL_VIDEO_ID.length > 5) {
    iframe.src = 'https://www.youtube.com/embed/' + MANUAL_VIDEO_ID + '?rel=0&modestbranding=1&origin=' + window.location.origin;
    return;
  }
...
function spinSlotMachine() {
  console.log('=== SPIN INICIADO === Intentos antes:', dailySpinsLeft, 'isSpinning:', isSpinning);
  
  if (isSpinning) {
    console.log('Ya está girando, ignorando click');
    return;
  }
  ...
  // Ahora sí restar intento
  dailySpinsLeft = dailySpinsLeft - 1;
  console.log('Intento usado. Quedan:', dailySpinsLeft);
  ...
  setTimeout(function() {
    try {
      ...
      console.log('Error en resultado:', e);
    }
    
    // SIEMPRE actualizar el botón al final
    isSpinning = false;
    var spinBtn = document.getElementById('spinButton');
    console.log('Actualizando botón. Intentos:', dailySpinsLeft);
    
    if (dailySpinsLeft > 0) {
      spinBtn.disabled = false;
      spinBtn.innerHTML = '🎰 GIRAR (' + dailySpinsLeft + '/2)';
    } else {
      spinBtn.disabled = true;
      spinBtn.innerHTML = '❌ SIN INTENTOS';
    }
  }, 4500);
}
...
function toggleTerminosSection(header) {
  const section = header.parentElement;
  section.classList.toggle('collapsed');
}

// Inicializar secciones de términos como expandidas
document.addEventListener('DOMContentLoaded', () => {
  // Opcional: colapsar todas las secciones excepto la primera al cargar
  const terminosSections = document.querySelectorAll('.terminos-section');
  terminosSections.forEach((section, index) => {
    if (index > 0) {
      // section.classList.add('collapsed'); // Descomentar si quieres que empiecen colapsadas
    }
  });
});
...
const btnIniciarTwitch = document.getElementById('btnIniciarTwitch');
const closeTwitchModal = document.getElementById('closeTwitchModal');
const connectTwitchBtn = document.getElementById('connectTwitchBtn');
const linkTerminos = document.getElementById('linkTerminos');

// Abrir modal al hacer clic en "Iniciar"
if (btnIniciarTwitch && twitchModal) {
  btnIniciarTwitch.addEventListener('click', (e) => {
    e.preventDefault();
...
if (closeTwitchModal && twitchModal) {
  closeTwitchModal.addEventListener('click', () => {
...
// Botón de conectar con Twitch
if (connectTwitchBtn) {
  connectTwitchBtn.addEventListener('click', () => {
    // Aquí iría la lógica de OAuth con Twitch
    // Por ahora mostramos un mensaje
    alert('🔗 Redirigiendo a Twitch para autenticación...');
    // window.location.href = 'TU_URL_DE_OAUTH_TWITCH';
  });
}

// Link a términos y condiciones desde el modal
if (linkTerminos) {
  linkTerminos.addEventListener('click', (e) => {
    e.preventDefault();
    twitchModal.classList.remove('active');
    
    // Cambiar a la sección de términos
    updateTitle('terminos');
    
    document.querySelectorAll('.content-section, .news-section').forEach(s => {
      s.classList.remove('active');
    });
    
    const terminosSection = document.getElementById('section-terminos');
    if (terminosSection) terminosSection.classList.add('active');
  });
}

// Cerrar modal con tecla Escape
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && twitchModal && twitchModal.classList.contains('active')) {
    twitchModal.classList.remove('active');
  }
});